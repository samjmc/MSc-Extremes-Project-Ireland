---
title: "Threshold Method Exceedances"
...
gpd_pdf <- extRemes::devd(x_values_gpd,  scale = scale_threshold, shape = shape_threshold, type = "GP")

# Plot the GEV distribution using base R plotting functions
plot(x_values_gpd, gpd_pdf, type = "l", lwd = 2, col = "blue",
...
calculate_station_return_levels <- function(station_data) {
  ...
  return_levels <- return.level(gpd_fit, return.period = c(2, 5,20, 100))
...
tm_shape(ireland) +
  ...
  tm_dots(col = "return_2yr", size = 1.5, alpha = 0.5, title = "Return Level (2yr) Using Threshold") +
  tm_style("gray")
...
exceedance_data_threshold <-inner_join(data %>% select(station_name, year, month, day, surge_daily_max), return_levels_with_coords2, by = "station_name")
...
station_exceedances_20yr <- exceedance_data_threshold %>%
...
cluster_exceedance_summary_20yr_POT <- station_exceedances_20yr %>%
  group_by(cluster) %>%
...
# Print the summary
print(cluster_exceedance_summary_20yr_POT)

##FOR 5 YEAR EXCEEDANCES

# Calculate total exceedances for each station
station_exceedances2 <- exceedance_data_threshold %>%
  group_by(cluster, station_name) %>%
  summarise(total_exceedance_5yr = sum(surge_daily_max > return_5yr, na.rm = TRUE)) %>%
  ungroup()
# Summarize exceedances by cluster
cluster_exceedance_summary_POT <- station_exceedances2 %>%
  group_by(cluster) %>%
...
# Print the summary
print(cluster_exceedance_summary_POT)

##FOR 2-YEAR EXCEEDANCES

# Calculate total exceedances for each station
station_exceedances_2yr <- exceedance_data_threshold%>%
  group_by(cluster, station_name) %>%
  summarise(total_exceedance_2yr = sum(surge_daily_max > return_2yr, na.rm = TRUE)) %>%
  ungroup()
# Summarize exceedances by cluster
cluster_exceedance_summary_2yr_POT <- station_exceedances_2yr %>%
  group_by(cluster) %>%
...
# Print the summary
print(cluster_exceedance_summary_2yr_POT)
...
annual_exceedance_counts2 <- exceedance_data_threshold %>%
...
# Step 2: Summarize total exceedances above the 5-year return level for each station and year
annual_exceedance_counts_2yr_threshold <- exceedance_data_threshold %>%
  group_by(cluster, station_name, year) %>%
  summarise(exceedance_2yr = sum(surge_daily_max > return_2yr, na.rm = TRUE)) %>%
  ungroup()

# Check the summarized data
print(annual_exceedance_counts_2yr_threshold)

# Step 3: Order stations by cluster and station name
station_order <- annual_exceedance_counts_2yr_threshold %>%
  arrange(cluster, station_name) %>%
  pull(station_name) %>%
  unique()

# Convert station_name to factor with the specified order
annual_exceedance_counts_2yr_threshold$station_name <- factor(annual_exceedance_counts_2yr_threshold$station_name, levels = station_order)

# Step 4: Create the heatmap plot including every year label and smaller facet labels
heatmap_plot_2yr_threshold <- ggplot(annual_exceedance_counts_2yr_threshold, aes(x = year, y = station_name, fill = exceedance_2yr)) +
  geom_tile() +
  scale_fill_gradient(low = "lightblue", high = "darkblue", name = "Exceedances") +
  labs(title = "Exceedances Above 2-Year Return Level Heatmap (Threshold)",
       x = "Year", y = "Station") +
  theme(
    axis.text.y = element_text(size = 0),  # Adjust y-axis text size
    axis.text.x = element_text(angle = 45, hjust = 1, size = 7),  # Rotate and adjust x-axis text size
  ...
  scale_x_continuous(breaks = seq(min(annual_exceedance_counts_2yr_threshold$year), max(annual_exceedance_counts_2yr_threshold$year), by = 1)) +  # Label every year
  facet_grid(cluster ~ ., scales = "free_y", space = "free_y")  # Separate clusters into facets

# Print the plot
print(heatmap_plot_2yr_threshold)
```
...
tm_shape(ireland) +
  tm_borders() +
  tm_shape(return_levels_sf_clusters2) +
  tm_dots(col = "cluster", palette = custom_colors2, size = 1, alpha = 0.5, title = "Return Level Clusters (2yr, Threshold Method)", title.size = 0.5) + 
  tm_style("gray")
```
...
annual_exceedance_counts_threshold <- exceedance_data_threshold  %>% 
                              group_by(station_name, year) %>% 
                              summarise(exceedance_2yr = sum(surge_daily_max > return_2yr),
                                        exceedance_20yr = sum(surge_daily_max > return_20yr)) %>% 
                              ungroup()

## plot for DL 
ggplot(annual_exceedance_counts_threshold  %>% filter(station_name == "795"), aes(x = year, y = exceedance_2yr)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  labs(title = "Exceedances Above 2-Year Return Level Per Year",
       x = "Year", y = "Exceedances")
```
